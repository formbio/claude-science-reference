...
| `host.llm(prompt_or_list, model=?, ...)` | `python` | Single-turn completion via the host's API client. Defaults to Haiku (claude-haiku-4-5, unless the deployment overrides `[llm] kernel_default_model`); pass `model=` for harder reasoning — heavier models cost 10-30× more |
