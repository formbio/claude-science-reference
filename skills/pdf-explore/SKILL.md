...
yourself are different: bare calls use the kernel default, which
defaults to Haiku (claude-haiku-4-5) but can be overridden via
`[llm] kernel_default_model` — check `help(host.llm)` if cost
matters there.) Don't
